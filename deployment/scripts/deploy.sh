...
check_prerequisites() {
    ...
    local has_app=$(gcloud container images list --repository=gcr.io/$project_id 2>/dev/null | grep -c "bigdata-app" || echo "0")
    local has_spark=$(gcloud container images list --repository=gcr.io/$project_id 2>/dev/null | grep -c "bigdata-spark" || echo "0")

    if [ "$has_app" -gt 0 ] && [ "$has_spark" -gt 0 ]; then
        log_success "Docker images found in GCR (bigdata-app, bigdata-spark)"
    else
        log_warning "Docker images not found in GCR. Please build and push first:"
...
main() {
    ...
    kubectl apply -f k8s/kafka-producer-crawl-deployment.yaml
    wait_for_pods "kafka-producer" ${NAMESPACE}
    
    # 5.3 HDFS Archiver (CronJob)
    log_info "Creating HDFS Archiver CronJob..."
    kubectl apply -f k8s/hdfs-archiver-cronjob.yaml
    log_success "HDFS Archiver CronJob created"

    # 5.4 Kafka -> Spark bridge
    log_info "Deploying Kafka -> Spark bridge..."
    kubectl apply -f k8s/kafka-spark-bridge-deployment.yaml
    wait_for_pods "kafka-spark-bridge" ${NAMESPACE}

    # 5.5 Spark Streaming (metrics)
    log_info "Deploying Spark Streaming (metrics)..."
    kubectl apply -f k8s/spark-streaming-consumer-deployment.yaml
    wait_for_pods "spark-streaming" ${NAMESPACE}

    # 5.6 Spark Streaming (alerts)
    log_info "Deploying Spark Streaming (alerts)..."
    kubectl apply -f k8s/spark-alerts-deployment.yaml
    wait_for_pods "spark-streaming-alerts" ${NAMESPACE}

    # 5.7 Spark Batch (features) CronJob
    log_info "Creating Spark Batch Features CronJob..."
    kubectl apply -f k8s/spark-batch-features-cronjob.yaml
    log_success "Spark Batch Features CronJob created"
    
    log_success "All applications deployed"
    
    # Step 6: Verify CronJobs
    log_step "⏰ STEP 6: VERIFYING CRONJOBS"
    log_info "CronJobs in namespace ${NAMESPACE}:"
